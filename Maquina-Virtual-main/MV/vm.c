...
void interpretaInstruccion(uint8_t instruccion) {
    uint8_t tipoOperandoB = (instruccion >> 6) & 0x03; // 0b00000011
    uint8_t tipoOperandoA = (instruccion >> 4) & 0x03;
    uint8_t operationCode = instruccion & 0x1F;         // 0b00011111

    printf("Operando A: %X\n", tipoOperandoA);
    printf("Operando B: %X\n", tipoOperandoB);
    printf("Código de operación: %X\n", operationCode);
}